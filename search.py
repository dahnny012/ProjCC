...
class search():
	...
	def readFile(self, filename=None):
		...
		# Check for existing output and append a number to the filename if necessary
		# Write the header row to the csv
		self.filename = self.checkFilename(self.filename, ".csv")
		self.writeRow(self.filename + ".csv", ["SERIES_ID", "SERIES_NAME", "VOLUME", "CHAPTER", "GROUP_NAME", "GROUP_ID"])
	...
	def checkFilename(self, filename, fileExt):
		if os.path.isfile(filename + fileExt):
			count = 0
			while True:
				count += 1
				countStr = str(count)
				if not os.path.isfile(filename + countStr + fileExt):
  ...
  # Mimic csv.writer().writeRow(), since csv isn't compatible with codecs.open()
	...
	def lockWrite(self, sId, lock, fileExt):
		with lock:
			with open(self.filename + fileExt, 'a') as outFile:
				outFile.write(sId + "\n")
